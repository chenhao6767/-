...
class MainActivity : AppCompatActivity() {
    ...
    private fun showSoundSettings() {
        val items = arrayOf(
            "✅ 匹配成功提示音：${if (SoundAlertManager.successEnabled) "已开启" else "已关闭"}",
            "❌ 未匹配提示音：${if (SoundAlertManager.failureEnabled) "已开启" else "已关闭"}",
            "⏹ 停止运行提示音：${if (SoundAlertManager.stopEnabled) "已开启" else "已关闭"}",
            "🔊 测试成功音效",
            "🔇 测试失败音效"
        )
        android.app.AlertDialog.Builder(this)
            ...
            .setItems(items) { _, which ->
                when (which) {
                    0 -> {
                        SoundAlertManager.successEnabled = !SoundAlertManager.successEnabled
                        Toast.makeText(this, "匹配成功音：${if (SoundAlertManager.successEnabled) "已开启" else "已关闭"}", Toast.LENGTH_SHORT).show()
                    }
                    1 -> {
                        SoundAlertManager.failureEnabled = !SoundAlertManager.failureEnabled
                        Toast.makeText(this, "未匹配音：${if (SoundAlertManager.failureEnabled) "已开启" else "已关闭"}", Toast.LENGTH_SHORT).show()
                    }
                    2 -> {
                        SoundAlertManager.stopEnabled = !SoundAlertManager.stopEnabled
                        Toast.makeText(this, "停止音：${if (SoundAlertManager.stopEnabled) "已开启" else "已关闭"}", Toast.LENGTH_SHORT).show()
                    }
                    3 -> SoundAlertManager.playSuccess()
                    4 -> SoundAlertManager.playFailure()
                }
            }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == ScreenCaptureHelper.REQUEST_CODE && resultCode == RESULT_OK && data != null) {
            // Android 10+ requires MediaProjection inside a foreground service
            ...
            appendLog("📷 截图权限已授予，图像识别就绪")
            if (pendingCropIndex >= 0) {
                // Give the service a moment to initialise before cropping
    ...
    private fun startClicking() {
        ...
        // Start background foreground service so OS won't kill us
        val svcIntent = Intent(this, BackgroundRunnerService::class.java)
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            startForegroundService(svcIntent)
        } else {
            startService(svcIntent)
    ...
    private fun stopClicking() {
        ...
        updateUI()
        appendLog("⏹ 已停止")
        stopService(Intent(this, BackgroundRunnerService::class.java))
    }
